    ...
    for question in questions:
        ...
        for answer in questions[question]:
            if not auto:
                ...
                d[f'{question}_{answer}_concentration'] = \
                    tfds.features.Sequence(tf.float64, length=25)
...
class GalaxyZooDecals(tfds.core.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        ...
        GalaxyZooDecalsConfig(name='volunteers_5',
                              data='5',
                              csv_name='gz_decals_volunteers_5.csv'
                              ),
        GalaxyZooDecalsConfig(name='auto',
                              data='5',
    ...
    def morphology_features(self):
        ...
        if self.builder_config.data is '5':
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        image_paths = [
            data_path / f'gz_decals_dr5_png_part{i}'
            for i in range(1, 5)
        ]
        return {
        ...
        with csv_path.open() as f:
            for row in csv.DictReader(f):
                ...
                if image_path:
                    yield iauname, {
                        'image': image_path,
                        'morphology': {k: 'nan' if row[k] == '' else row[k]
                                       for k in self.morphology_features},
                        'metadata': {k: 'nan' if row[k] == '' else row[k] for
                                     k in _METADATA},
                    }
